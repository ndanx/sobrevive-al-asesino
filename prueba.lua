...
-------------------------------------------------------------------
-- LÓGICA DE AUTO REVIVE (DETECCIÓN POR ESTADO Y BARRA DE AYUDA)
-------------------------------------------------------------------
task.spawn(function()
    while true do
        task.wait(0.1)
            ...
            if root then
                ...
                for _, p in pairs(game.Players:GetPlayers()) do
                    if p ~= lp and p.Character then
                        ...
                        -- NUEVA DETECCIÓN: 
                        -- 1. Busca el BillboardGui (la barrita de "HELP") que aparece sobre la cabeza
                        -- 2. Busca valores como "Downed" o "HelpMe" dentro del personaje
                        local hasHelpSign = char:FindFirstChild("Head") and char.Head:FindFirstChildOfClass("BillboardGui")
                        local isDownedValue = char:FindFirstChild("Downed") or char:FindFirstChild("IsDowned") or char:FindFirstChild("Status")
                        
                        -- Si detecta el signo de ayuda o el estado de caído
                        if (hasHelpSign or isDownedValue) and tRoot then
                            -- Regla de los 5 studs
...
Tabs.Movement:AddToggle("ReviveT", {
    Title = "Auto Revive (Smart Detect)",
    Description = "TP por barra de 'HELP' + Seguridad 5 studs",
    Default = false
