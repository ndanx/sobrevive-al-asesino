-- MANTENEMOS TU ESTRUCTURA ORIGINAL DE FLUENTPLUS
...
-------------------------------------------------------------------
-- LÓGICA DESCIFRADA DE AUTO REVIVE (AKA HUB LOGIC)
-------------------------------------------------------------------
task.spawn(function()
    while true do
        task.wait(0.1)
        if Options.AutoRevive then
            pcall(function()
                ...
                if lpRoot then
                    -- 1. Buscamos al Killer (Mayúscula en K)
                    ...
                    -- 2. Buscamos sobrevivientes con el estado real
                    for _, p in pairs(game.Players:GetPlayers()) do
                        if p ~= lp and p.Character then
                            local char = p.Character
                            
                            -- ESTA ES LA DETECCIÓN REAL DEL JUEGO:
                            -- Busca el valor 'Downed' en el modelo o 'HelpSymbol' en la cabeza
                            local isDowned = char:FindFirstChild("Downed") 
                            local hasHelpSign = char:FindFirstChild("Head") and char.Head:FindFirstChild("HelpSymbol")

                            if isDowned or hasHelpSign then
                                local tRoot = char:FindFirstChild("HumanoidRootPart")
                                if tRoot then
                                    -- Seguridad de 5 studs respecto al Killer
                                    local distToKiller = killerRoot and (tRoot.Position - killerRoot.Position).Magnitude or 100
                                    
                                    if distToKiller >= 5 then
                                        lpRoot.CFrame = tRoot.CFrame * CFrame.new(0, 0, 1)
                                        task.wait(0.5) -- Pausa para que no rebote
                                        break 
                                    end
                                end
                            end
...
-------------------------------------------------------------------
-- EL RESTO DE TU CONFIGURACIÓN (ESP, SPEED, ETC.)
-------------------------------------------------------------------
...
Tabs.Movement:AddToggle("ReviveT", {
    Title = "Auto Revive (Decoded)",
    Description = "Usa 'HelpSymbol' y 'Downed' para detectar heridos",
    Default = false
...
-- Mantener la lógica de Stepped para Speed y ESP
