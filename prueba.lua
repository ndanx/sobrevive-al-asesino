...
-- AGREGADO: Toggle de Auto Revive
Tabs.Movement:AddToggle("ReviveT", {
    Title = "Auto Revive", 
    Description = "Revive a compañeros automáticamente."
...
RunService.Stepped:Connect(function()
    ...
    for _, p in pairs(Players:GetPlayers()) do
        if p ~= player and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
            ...
            local color = nil
            if Options.KillerESP and p.Team and p.Team.Name == "Killer" then color = Color3.fromRGB(255,0,0)
            elseif Options.SurvivorESP and p.Team and p.Team.Name == "Survivor" then color = Color3.fromRGB(0,255,255) end
            ...
            -- AGREGADO: Lógica de Auto Revive
            if Options.AutoRevive and p.Team and p.Team.Name == "Survivor" then
                if p.Character.Humanoid.Health <= 5 then -- Si está en el suelo
                    local dist = (player.Character.HumanoidRootPart.Position - p.Character.HumanoidRootPart.Position).Magnitude
                    if dist < 15 then
                        -- Usa la carpeta Events que encontraste
                        local revive = ReplicatedStorage.Events:FindFirstChild("Revive") or ReplicatedStorage.Events:FindFirstChild("RevivePlayer")
                        if revive then
                            revive:FireServer(p.Name)
                        end
                    end
                end
            end
...
Window:SelectTab(1)
Fluent:Notify({Title = "Survive The Killer", Content = "Auto Revive agregado correctamente.", Duration = 5})
