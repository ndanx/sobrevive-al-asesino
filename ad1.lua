...
-- 1. Crear el Toggle con la estructura de la imagen
local MinMaxToggle = Tabs.Settings:AddToggle("MinMaxToggle", {
    Title = "Modo Pantalla Completa",
    Description = "Alterna entre Maximize (On) y Minimize (Off)",
    Default = false
})

-- 2. Manejo de eventos con OnChanged como en tu imagen
MinMaxToggle:OnChanged(function()
    local state = Options.MinMaxToggle.Value
    if state then
        Window:Maximize() -- Activa el tamaño expandido (Imagen 1)
    else
        Window:Minimize() -- Activa el modo contraído (Imagen 2)
    end
end)

-- 3. Keybind para la tecla de menú (G)
Tabs.Settings:AddKeybind("MinimizeKeybind", {
    Title = "Tecla de Menú",
    Default = "G",
    ChangedCallback = function(NewKey)
        Options.MenuKey = NewKey
