...
Tabs.Settings:AddKeybind("WindowToggleKey", {
    Title = "Alternar Tamaño de Ventana",
    Description = "Elige una tecla para alternar entre Maximize y Minimize",
    Default = "F8", -- Puedes cambiarla luego en el menú
    
    -- Usamos el Callback para que funcione como un interruptor (Toggle)
    Callback = function()
        IsMaximized = not IsMaximized -- Cambia el estado cada vez que presionas
        
        if IsMaximized then
            Window:Maximize() -- Si es true, expande
        else
            Window:Minimize() -- Si es false, contrae
        end
    end,

    -- ChangedCallback por si quieres guardar la tecla en tus opciones
    ChangedCallback = function(NewKey)
        print("Nueva tecla para el Toggle de ventana:", NewKey)
    end
})
