...
-- Tecla para minimizar y abrir hub

Tabs.Settings:AddKeybind("WindowActionKey", {
    Title = "Keybind",
    Description = "Abre y cierra el menú manteniendo su tamaño",
    Default = "G", 
    Callback = function()
        -- Localizamos el marco principal del Hub
        local MainFrame = Window.Root:FindFirstChild("Main")
        
        if MainFrame then
            -- Alternamos la visibilidad
            -- Esto hace que el menú aparezca exactamente donde lo dejaste
            MainFrame.Visible = not MainFrame.Visible
        end
    end,
    ChangedCallback = function(NewKey)
        print("Nueva tecla asignada:", NewKey)
    end
})
