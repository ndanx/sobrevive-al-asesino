...
Tabs.Settings:AddKeybind("WindowActionKey", {
    Title = "Keybind",
    Description = "Abre/Cierra el menú sin deformar el tamaño",
    Default = "G", 
    Callback = function()
        -- Localizamos el marco principal de la interfaz
        local MainFrame = Window.Root:FindFirstChild("Main")
        
        if MainFrame then
            -- Alternamos la visibilidad directamente
            -- Esto evita los errores "missing method" y "boolean value"
            MainFrame.Visible = not MainFrame.Visible
        end
    end,
    ChangedCallback = function(NewKey)
        print("Nueva tecla asignada para el menú:", NewKey)
    end
})
